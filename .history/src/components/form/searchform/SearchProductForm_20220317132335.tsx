...
function SearchProductForm() {
    ...
    const expandAndCollapse = () =>{
        ...
        setHeight((active) => (active ? '0px' : `${contentSpace.current.scrollHeight}px`))
        setRotate((active) => (active ? 'transform duration-700 ease' : 'transform duration-700 ease rotate-180'))
    }
  ...
  return (
    ...
    <div className='bg-[#323259] w-full px-[20px] pt-[20px]'>
        <div className='grid grid-cols-12 gap-3 h-[62px] text-white '>
            ...
            <div className='col-span-3'>
                <select className=' w-full h-[38px] py-[7px] px-[16px] outline-none bg-[#252547] rounded-sm'>
                    <option value="ka">Any stock status</option>
                    <option value="ka">In stock</option>
                    <option value="ka">Low Stock</option>
                    <option value="ka">SOLD</option>
                </select>
            </div>
        ...
        <div className={`flex relative transition-maxHeight duration-700 ease-in-out overflow-auto`} ref={contentSpace} style={{maxHeight:`${height}`}}>
    ...
    <div className='flex justify-center'>
        <div onClick={expandAndCollapse} className=' w-[50px] bg-[#323259] text-center'>
            <KeyboardArrowDown className={`h-full cursor-pointer transition-transform ${rotate}`}/>
        </div>
    </div>
    </Fragment>
  )
