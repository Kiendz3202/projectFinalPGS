...
function SearchProductForm() {
    ...
    const searchForm = (e:any) =>{
        ...
        if(keywordFilterRef.current.value === '' && categoryValueSearch !== ''){
            dispatch(productsActions.SearchProductForm({
                isSearching:true,
                data:productsStore?.filter((product) =>  product?.description?.includes(categoryValueSearch))
            }))
        }
        if(keywordFilterRef.current.value !== '' && categoryValueSearch !== ''){
            dispatch(productsActions.SearchProductForm({
                isSearching:true,
                data:productsStore?.filter((product) =>(product?.sku?.includes(keywordFilterRef.current.value) || product?.name?.includes(keywordFilterRef.current.value)) && (product?.description?.includes(categoryValueSearch)) )
            }))
        }
    }
