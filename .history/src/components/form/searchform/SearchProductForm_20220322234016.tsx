...
function SearchProductForm(){
    ...
    const searchForm = (e:any) =>{
        ...
        if(formFilterValue.keyword !== ''){
            dataRender = productsSearching?.filter((product) => product?.sku?.includes(formFilterValue.keyword) || product?.name?.includes(formFilterValue.keyword) || product?.category?.includes(formFilterValue.keyword) || product?.description?.includes(formFilterValue.keyword))
            dispatch(productsActions.SearchProductForm({
                isSearching:true,
                data:dataRender
            }))
        }
        if(formFilterValue.category !== 'anycategory'){
            dataRender = productsSearching.filter((product) =>  product?.category?.includes(categoryValueSearch))
            dispatch(productsActions.SearchProductForm({
                isSearching:true,
                data:dataRender
            }))
