...
function SearchProductForm() {
    ...
    const [categoryValueSearch,setCategoryValueSearch] = useState('')
    ...
    const searchForm = (e:any) =>{
        if(keywordFilterRef.current.value === '' && categoryValueSearch === ''){
        ...
        if(keywordFilterRef.current.value !== '' && categoryValueSearch === ''){
            dispatch(productsActions.SearchProductForm({
                isSearching:true,
                data:productsStore?.filter((product) => product?.sku?.includes(keywordFilterRef.current.value) || product?.name?.includes(keywordFilterRef.current.value) || product?.description?.includes(keywordFilterRef.current.value))
            }))
        }
        // if(keywordFilterRef.current.value === '' && categoryValueSearch !== ''){
        //     dispatch(productsActions.SearchProductForm({
        //         isSearching:true,
        //         data:productsStore?Filter((product) => product?.sku?.includes(categoryValueSearch) || product?.name?.includes(categoryValueSearch) || product?.description?.includes(categoryValueSearch))
        //     }))
        // }
        // if(keywordFilterRef.current.value !== '' && categoryValueSearch !== ''){
        //     dispatch(productsActions.SearchProductForm({
        //         isSearching:true,
        //         data:productsStore?.filter((product) =>(product?.sku?.includes(keywordFilterRef.current.value) || product?.name?.includes(keywordFilterRef.current.value) || product?.description?.includes(keywordFilterRef.current.value)) && (product?.sku?.includes(categoryValueSearch) || product?.name?.includes(categoryValueSearch) || product?.description?.includes(categoryValueSearch)) )
        //     }))
        // }
    }
