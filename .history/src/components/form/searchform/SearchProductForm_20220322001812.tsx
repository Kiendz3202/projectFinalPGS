...
function SearchProductForm() {
    ...
    const searchForm = (e:any) =>{
        if(keywordFilterRef.current.value === '' && categoryValueSearch === 'anycategory' && stockValueSearch === 'anystock'){
            ...
            setDataRender(productsStore)
        }
        if(keywordFilterRef.current.value !== ''){
            setDataRender((prev) => prev.filter((product) => product?.sku?.includes(keywordFilterRef.current.value) || product?.name?.includes(keywordFilterRef.current.value) || product?.category?.includes(keywordFilterRef.current.value) || product?.description?.includes(keywordFilterRef.current.value)))
            dispatch(productsActions.SearchProductForm({
                isSearching:true,
                data:dataRender
            }))
        ...
        if(categoryValueSearch !== 'anycategory'){
            setDataRender((prev) => prev.filter((product) =>  product?.category?.includes(categoryValueSearch)))
            dispatch(productsActions.SearchProductForm({
                isSearching:true,
                data:dataRender
            }))
