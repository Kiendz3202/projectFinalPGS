...
function SearchProductForm() {
    ...
    const searchForm = (e:any) =>{
        ...
        if(keywordFilterRef.current.value !== '' && categoryValueSearch !== 'anycategory'){
            dispatch(productsActions.SearchProductForm({
                isSearching:true,
                data:productsStore?.filter((product) =>(product?.sku?.includes(keywordFilterRef.current.value) || product?.name?.includes(keywordFilterRef.current.value)) && (product?.category?.includes(categoryValueSearch)) )
            }))
        }
    }
