...
function ProductList() {
  ...
  return (
    <div className='bg-[#323259]'>
        ...
        {items && items?.map((item) => <ProductItem key={item.id}  sku={item.sku} name={item.name} category={item.category} price={item.price} amount={item.amount} vendor={item.vendor} arrivalDate={item.arrivalDate} />)}
        <NavProductList />
    </div>
  )
}
