...
function ProductList() {
  ...
  return (
    <div className='bg-[#323259]'>
        ...
        {items.map((item) => <ProductItem sku={item.sku} name={item.name} category={item.category} price={item.price} amount={item.amount} vendor={item.vendor} arrivalDate={item.arrivalDate} />)}
        <NavProductList />
    </div>
  )
}
