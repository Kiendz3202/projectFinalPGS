...
function ProductList() {
  ...
  const fetched = useSelector((state:RootState) => state.products.fetched)
  ...
  const fetchItems = async () =>{
    try{
      ...
      if(fetched){
        ...
        dispatch(productsActions.fetched())
      }
    }catch(err){
  ...
  const removeHandle = (e:any) =>{
    ...
    if(isSelectedAll){
      data = products.filter((product:fetchResponse) => !pagination.currentData.find(crr => crr.id === product.id))
      dispatch(productsActions.products({
        data
      }))
  ...
  return (
    <div className='bg-[#323259]'>
        ...
        {pagination.currentData && pagination.currentData.map((item) => <ProductItem isSelectedAll={isSelectedAll} key={item.id} id={item.id}  sku={item.sku} name={item.name} category={item.category} price={item.price} amount={item.amount} vendor={item.vendor} arrivalDate={item.arrivalDate} />)}
