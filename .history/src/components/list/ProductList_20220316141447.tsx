...
function ProductList() {
  ...
  useEffect(() => {
    const respone = fetch('https://api.gearfocus.div4.pgtest.co/api/products/list')
    const itemslist = respone.json()
    setPagination((prev) => ({
      ...prev,
      data:itemslist.data
    }))
  ...
  return (
    <div className='bg-[#323259]'>
        ...
        {pagination && pagination.currentData.map((item) => <ProductItem key={item.id}  sku={item.sku} name={item.name} category={item.category} price={item.price} amount={item.amount} vendor={item.vendor} arrivalDate={item.arrivalDate} />)}
