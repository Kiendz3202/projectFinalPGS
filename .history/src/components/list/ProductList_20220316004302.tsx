...
function ProductList() {
  const [items,setItems] = useState<paginationType>()
  ...
  useEffect(() =>{
    ...
    //   currentData: prevState.data.slice(items.offset, items.offset + items.numberPerPage)
    // }))
  },[])
  return (
    <div className='bg-[#323259]'>
        ...
        {items && items.data.map((item,index) => <ProductItem id={item.id} sku={item.sku} name={item.name} category={item.category} price={item.price} amount={item.amount} vendor={item.vendor} arrivalDate={item.arrivalDate} />)}
        <NavProductList />
    </div>
  )
}
