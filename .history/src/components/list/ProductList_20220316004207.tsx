...
function ProductList() {
  ...
  return (
    <div className='bg-[#323259]'>
        ...
        {items?.data.map((item,index) => <ProductItem id={item.id} sku={item.sku} name={item.name} category={item.category} price={item.price} amount={item.amount} vendor={item.vendor} arrivalDate={item.arrivalDate} />)}
        <NavProductList />
    </div>
  )
}
