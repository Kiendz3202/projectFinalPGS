...
function ProductList() {
  ...
  return (
    <div className='bg-[#323259]'>
        ...
        {items.map((item) => <ProductItem sku={item.sku} name={item.name} category={item.category} price={item.price} instock={item.amount} vendor={item.vendor} arrivalDate={item.arrivalDate} />)}
        {/* <ProductItem  /> */}
    </div>
  )
}
