...
function DetailProductPage() {
  ...
  const printf = () =>{
    ...
    productsData[productChangedIndex] = ({vendor:product?.vendor},{name:product?.name},{sku:product?.sku},{category:product?.category},{description:product?.description},{price:product?.price},{arrivalDate:product?.arrivalDate},{amount:product?.amount})
  }
  ...
  return (
    <div className='bg-[#1b1b38] w-full h-full ml-[256px] mt-[76px] p-[36px] text-white'>
      ...
      <div className='flex items-center w-full h-[75px] bg-[#323259] px-[36px] py-[15px]' style={{boxShadow:'0 0 13px 0 #b18aff'}}>
        <button onClick={printf} className='flex items-center bg-[#f0ad4e] hover:opacity-80 w-[148px] h-[33px] px-[15px] py-[8px] rounded-md cursor-pointer'>Update product</button>
      </div>
    </div>
  )
}
